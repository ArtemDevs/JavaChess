...
public class Queen extends Piece {
  private static final int[] CANDIDATE_MOVE_VECTOR_CORDS = {-9, -8, -7, -1, 1, 7, 8, 9};

  public Queen(int piecePosition, Loyalty pieceLoyalty) {
    ...
    for (final int candidateCordOffset : CANDIDATE_MOVE_VECTOR_CORDS) {
      ...
      while (BoardUtils.isValidSquareCord(candidateDestinationCord)) {
        ...
        if (BoardUtils.isValidSquareCord(candidateDestinationCord)) {
          ...
          // if the square is not occupied then we add a legal move to the list, and we can loop
          // again
          if (!candidateDestinationSquare.isSquareOccupied()) {
            ...
            if (this.pieceLoyalty != pieceLoyalty) {
              legalMoves.add(
                  new Move.AttackMove(
                      board, this, candidateDestinationCord, pieceAtDestinationCord));
            }
            // if there is a friendly piece that is blocking the rest of the vector, we shouldn't
            // consider any
